  ...
  return (
    <div className="space-y-8 pb-16">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-taupe/20 pb-6">
        <div>
          <h1 className="text-3xl font-serif text-obsidian tracking-tight">Product Catalog</h1>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="inline-flex items-center justify-center rounded-md border border-taupe/20 bg-white px-3.5 py-2 text-sm font-medium text-charcoal shadow-sm hover:bg-warm-white transition"
            title="Refresh product list"
          >
            <RefreshCw className={`h-4 w-4 ${loading ? "animate-spin text-taupe" : "text-charcoal"}`} />
          </button>
          <Link
            href="/admin/products/new"
            className="inline-flex items-center justify-center rounded-md bg-obsidian px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-charcoal transition"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="rounded-xl border border-taupe/20 bg-white p-5 shadow-xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-taupe">Total Products</span>
            <Package className="h-4 w-4 text-taupe" />
          </div>
          <div className="mt-2 text-2xl font-serif text-obsidian font-medium">{totalCount}</div>
          <div className="text-xs text-taupe mt-1">Items in catalog database</div>
        </div>

        <div className="rounded-xl border border-taupe/20 bg-white p-5 shadow-xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-taupe">Live in Store</span>
            <CheckCircle2 className="h-4 w-4 text-emerald-600" />
          </div>
          <div className="mt-2 text-2xl font-serif text-emerald-700 font-medium">{publishedCount}</div>
          <div className="text-xs text-taupe mt-1">Visible to buyers</div>
        </div>

        <div className="rounded-xl border border-taupe/20 bg-white p-5 shadow-xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-taupe">Drafts</span>
            <Clock className="h-4 w-4 text-amber-600" />
          </div>
          <div className="mt-2 text-2xl font-serif text-amber-700 font-medium">{draftCount}</div>
          <div className="text-xs text-taupe mt-1">Unpublished products</div>
        </div>

        <div className="rounded-xl border border-taupe/20 bg-white p-5 shadow-xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-taupe">Low Stock</span>
            <AlertCircle className="h-4 w-4 text-rose-500" />
          </div>
          <div className="mt-2 text-2xl font-serif text-rose-700 font-medium">{lowStockCount}</div>
          <div className="text-xs text-taupe mt-1">Variants with ≤ 5 units left</div>
        </div>
      </div>

      {/* Filter and Search Bar */}
      <div className="rounded-xl border border-taupe/20 bg-white p-4 shadow-xs flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        <div className="relative flex-1">
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-taupe" />
          <input
            type="text"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            placeholder="Search by product name, SKU, or slug..."
            className="w-full rounded-md border border-taupe/20 bg-warm-white/30 py-2 pl-9 pr-4 text-sm text-obsidian placeholder:text-taupe/60 focus:border-obsidian focus:bg-white focus:ring-1 focus:ring-obsidian"
          />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2.5">
            ...
            {["ALL", "PUBLISHED", "DRAFT"].map((status) => (
              <button
                ...
                className={`px-3 py-1.5 rounded transition ${
                  statusFilter === status
                    ? "bg-white text-obsidian shadow-xs font-semibold"
          ...
          <select
            ...
            className="rounded-md border border-taupe/20 bg-white py-1.5 px-3 text-xs font-medium text-charcoal focus:border-obsidian focus:ring-1 focus:ring-obsidian"
          >
      ...
      {/* Products Table */}
      <div className="rounded-xl border border-taupe/20 bg-white shadow-xs overflow-hidden">
        {loading ? (
          ...
          <div className="py-20 text-center px-4">
            ...
            <p className="text-xs text-taupe max-w-sm mx-auto mt-1 mb-6">
